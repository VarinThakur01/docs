...
const resultVal = await new Promise<any[]>((resolve, reject) => {
  client.exec(
    `"SELECT VEC_TEXT, VEC_META, TO_NVARCHAR(VEC_VECTOR) AS VEC_VECTOR FROM LANGCHAIN_DEMO_NEW_TABLE LIMIT 1"`,
    (err: Error, rows: any) => {
      if (err) {
        reject(err);
      } else {
...
rowVal.forEach((r) => {
  ...
  console.log(`VEC_VECTOR: ${r.VEC_VECTOR}`); // The vector
});
```
...
VEC_VECTOR: [-0.01989901,0.027851747,...]
```
...
rowOwnTable.forEach((r) => {
  ...
  console.log(`MY_VECTOR: ${r.MY_VECTOR}`); // The vector
});
```
